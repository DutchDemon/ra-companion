...
const MISSABLE_CONTEXT_RULES: Record<string, MissableContextRule> = {
  'buzz off': {
    requiredForm: 'human',
    stages: ['F_SP103'],
    forbidFlagsAny: ['faronTwilightStarted', 'buzzHiveDroppedHawk', 'buzzHiveDroppedSlingshot'],
  },
  'smiling politely': {
    requiredForm: 'human',
    stages: ['F_SP103'],
...
function liveRuleAllowsAchievement(achievement: any, ramBeat?: RamBeatContext) {
  ...
  const flags = { ...(ramBeat.storyFlags || {}), ...(ramBeat.stateFlags || {}) };
...
export function isFaronTearAchievement(achievement: any) {
  const text = achievementText(achievement);
  return /tears? of light/.test(text) && /return the light to faron/.test(text);
}

export function isFaronTearObjectiveRelevant(achievement: any, ramBeat?: RamBeatContext) {
  if (!isFaronTearAchievement(achievement) || !ramBeat?.live) return false;
  return ['F_SP108', 'R_SP108', 'D_SB10'].includes(String(ramBeat.stageCode || '').trim());
}

export function isFaronTearObjectiveActive(achievement: any, ramBeat?: RamBeatContext) {
  return isFaronTearObjectiveRelevant(achievement, ramBeat)
    && ramBeat?.storyFlags?.faronVesselObtained === true;
}
...
function strictBeatAchievementAllowed(achievement: any, context: TwilightPresenceContext, ramBeat?: RamBeatContext) {
  ...
  const explicitLiveRule = MISSABLE_CONTEXT_RULES[title];

  // v0.5.7: the Faron Vessel achievement is the active story objective while
  // the player is actually hunting Tears in Faron Woods. Its RA description
  // says Faron, not Faron Woods, so ordinary text/location matching misses it.
  if (isFaronTearObjectiveRelevant(achievement, ramBeat)) return true;

  // An explicit GZ2E01 RAM rule is authoritative for availability. This fixes
  // achievements such as Buzz Off! and Smiling Politely whose text never says
  // "Ordon Village" even though the save state proves they are actionable here.
  if (explicitLiveRule) return true;
  ...
  if (dungeon) {
    ...
    if (referencesKnownBoss(achievement, context)) {
      // Same-dungeon boss missables are safe to retain as a *coming-up* signal.
      // They are not promoted to the active warning bucket until the exact boss
      // stage is detected, which keeps missable warnings strict while preventing
      // Forest Temple challenges from disappearing during dungeon traversal.
      return isMissableAchievement(achievement);
    }
...
export function buildTwilightContext(achievements: any[], presenceMessage: string, ramBeat?: RamBeatContext): ContextBuckets {
  ...
  const faronTearObjectives = strictRamBeat
    ? rpLocked.filter((achievement) => isFaronTearObjectiveRelevant(achievement, ramBeat))
    : [];
  const activeFaronTearObjectives = faronTearObjectives.filter((achievement) => isFaronTearObjectiveActive(achievement, ramBeat));
  const upcomingFaronTearObjectives = faronTearObjectives.filter((achievement) => !isFaronTearObjectiveActive(achievement, ramBeat));
  ...
  if (context.label && context.boss) {
    const activeBossAchievements = rpLocked.filter((achievement) => referencesCurrentBoss(achievement, context));
    rpMissables = activeBossAchievements.filter((achievement) => isMissableAchievement(achievement)).slice(0, 2);
    current = activeBossAchievements.filter((achievement) => !isMissableAchievement(achievement)).slice(0, 2);
    ...
    const used = new Set([...current, ...rpMissables, ...otherBossAchievements]);
    ...
    comingUp = strictRamBeat
      ? bossRelated.filter((achievement) => isMissableAchievement(achievement)).slice(0, 1)
  ...
  const routeComingUp = route.nextProgression.filter((achievement) => {
    ...
    if (strictRamBeat && isFaronTearAchievement(achievement) && !isFaronTearObjectiveRelevant(achievement, ramBeat)) return false;
