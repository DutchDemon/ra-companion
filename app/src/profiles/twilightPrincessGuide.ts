...
const GUIDE: Record<string, GuideSeed> = {
  ...
  'pump up the king': guide(
    'Break an Ordon pumpkin, then pour water from any bottle onto the small sprout it leaves behind so the pumpkin regrows.',
    'Treat the opening Ordon Village pumpkin sequence as the safe window; complete it before advancing away from the opening village events.',
    'Any bottle counts for preparation even when it is currently filled; empty it and refill it with water if needed.',
  ),
