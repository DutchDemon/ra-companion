...
import './styles.css';
import { buildTwilightContext, earnedHardcore, isFaronTearAchievement, isFaronTearObjectiveActive } from './profiles/twilightPrincess';
...
function useSnapshot(intervalMs = 5000) {
  ...
  async function refresh(forceRa = false) {
    try {
      setSnapshot(await window.raCompanion.getSnapshot(forceRa));
      setError('');
    } catch (e: any) {
          ...
          <>
            ...
            {contextTotal > 0 ? (
              <div className="context-sections">
                ...
                {!companion.missables.length && !companion.current.length && !companion.comingUp.length && !deepLiveAchievements.length && (
