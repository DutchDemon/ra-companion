...
export function LiveContextCard() {
  const { effectiveRam, snapshot, ramLive, gameActive, ramRoom } = useAppView();
  ...
  return (
    <article className="v5-panel v5-live-context">
      ...
      <div className="v5-kv"><span>Profile</span><b>{gameActive ? 'Twilight Princess (GC · USA)' : 'None'}</b></div>
      ...
      <div className="v5-kv"><span>Bottles</span><b>{typeof effectiveRam?.bottleCount === 'number' ? effectiveRam.bottleCount : '—'}</b></div>
      <div className="v5-kv"><span>Actors nearby</span><b>{effectiveRam?.actors ? `${effectiveRam.actors.dog?.currentRoom || 0} dog · ${effectiveRam.actors.cucco?.currentRoom || 0} Cucco` : '—'}</b></div>
      <div className="v5-kv"><span>State engine</span><b className={effectiveRam?.gameStateEngine ? 'good' : ''}>{effectiveRam?.gameStateEngine ? 'GZ2E01 live' : 'Waiting'}</b></div>
  ...
  return (
    <>
      ...
      <article className="v5-panel v5-overlay-control-panel">
        ...
        <div className="v5-hotkeys">
          <span className={shortcutState?.overlayToggle.registered ? 'hotkey-ok' : 'hotkey-error'}><kbd>{shortcutState?.overlayToggle.accelerator.replace('CommandOrControl+', '').replaceAll('+', ' + ') || 'Ctrl + Shift + O'}</kbd> Show / Hide · {shortcutState?.overlayToggle.registered ? 'Active' : 'Unavailable'}</span>
          <span className={shortcutState?.clickThrough.registered ? 'hotkey-ok' : 'hotkey-error'}><kbd>{shortcutState?.clickThrough.accelerator.replace('CommandOrControl+', '').replaceAll('+', ' + ') || 'Ctrl + Shift + C'}</kbd> Click-through · {shortcutState?.clickThrough.registered ? 'Active' : 'Unavailable'}</span>
        </div>
      </article>
    </>
  );
  ...
  return (
    <>
      ...
      <form className="v5-panel v5-settings-form" onSubmit={saveSettings}>
        ...
        <div className="v5-account-connection">
