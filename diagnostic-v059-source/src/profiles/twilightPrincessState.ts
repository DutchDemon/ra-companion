import { earnedHardcore, isFaronTearAchievement, isFaronTearObjectiveActive, isMissableAchievement, type ContextBuckets } from './twilightPrincess';
...
export const TWILIGHT_GAME_STATE_ENGINE = 'tp-gz2e01-v2';
...
function deepStateForAchievement(achievement: any, ram?: Snapshot['ram']): TwilightAchievementState | null {
  ...
  if (tearQuest) {
    ...
    const progress = { current, target: 16, label: `${province[0].toUpperCase()}${province.slice(1)} Tears of Light`, source: 'ram' as const };

    if (!vesselOwned) {
      return state('upcoming', 'Coming soon · Vessel not received', 'upcoming', 'ram', 'deep', `Waiting for the ${province[0].toUpperCase()}${province.slice(1)} Vessel of Light.`, undefined);
    }
    ...
    if (current >= 16) {
      return state('pending', 'Vessel full · Returning light', 'active', 'ram', 'deep', `All 16 ${province[0].toUpperCase()}${province.slice(1)} Tears of Light are collected. Waiting for the RetroAchievements unlock if needed.`, progress);
    }
    return state('in_progress', `In progress · ${current}/16 Tears`, 'active', 'ram', 'deep', `${province[0].toUpperCase()}${province.slice(1)} Vessel of Light obtained.`, progress);
  }
