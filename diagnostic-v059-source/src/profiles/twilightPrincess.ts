...
function strictBeatAchievementAllowed(achievement: any, context: TwilightPresenceContext, ramBeat?: RamBeatContext) {
  ...
  if (isFaronTearObjectiveActive(achievement, ramBeat)) return true;
...
export function buildTwilightContext(achievements: any[], presenceMessage: string, ramBeat?: RamBeatContext): ContextBuckets {
  ...
  const scored = context.label
    ? achievements
        .map((achievement) => {
          ...
          const tearObjectiveScore = strictRamBeat && isFaronTearObjectiveActive(achievement, ramBeat) ? 500 : 0;
  ...
  const activeFaronTearObjectives = strictRamBeat
    ? rpLocked.filter((achievement) => isFaronTearObjectiveActive(achievement, ramBeat))
    : [];
  ...
  const routeComingUp = route.nextProgression.filter((achievement) => {
    ...
    if (strictRamBeat && isFaronTearAchievement(achievement) && !isFaronTearObjectiveActive(achievement, ramBeat)) return false;
