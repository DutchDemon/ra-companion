...
declare global {
  ...
  interface Window {
    raCompanion: {
      ...
      getConfig: () => Promise<{ username: string; hasApiKey: boolean; apiKeyEncrypted: boolean }>;
      checkForUpdates: (force?: boolean) => Promise<UpdateStatus>;
      installUpdate: () => Promise<UpdateStatus>;
      onUpdateStatusChanged: (callback: (status: UpdateStatus) => void) => () => void;
      saveConfig: (config: { username: string; apiKey: string }) => Promise<{ username: string; hasApiKey: boolean; apiKeyEncrypted: boolean }>;
