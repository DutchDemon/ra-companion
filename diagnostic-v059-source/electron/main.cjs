...
const shortcutState = {
  overlayToggle: { registered: false, accelerator: 'CommandOrControl+Shift+O', primary: 'CommandOrControl+Shift+O', fallback: 'CommandOrControl+Alt+O', usingFallback: false },
  clickThrough: { registered: false, accelerator: 'CommandOrControl+Shift+C', primary: 'CommandOrControl+Shift+C', fallback: 'CommandOrControl+Alt+C', usingFallback: false },
};
...
function registerShortcutBinding(key, primary, fallback, callback) {
  ...
  try {
    ...
    if (globalShortcut.register(primary, callback)) {
      binding.registered = true;
      return true;
    }
  } catch (error) {
    console.warn(`[shortcuts] Failed to register ${primary}:`, error);
  }

  try {
    globalShortcut.unregister(fallback);
    if (globalShortcut.register(fallback, callback)) {
      binding.registered = true;
    ...
    console.warn(`[shortcuts] Failed to register fallback ${fallback}:`, error);
  }
...
function registerGlobalShortcuts() {
  registerShortcutBinding('overlayToggle', 'CommandOrControl+Shift+O', 'CommandOrControl+Alt+O', () => toggleOverlay());
  registerShortcutBinding('clickThrough', 'CommandOrControl+Shift+C', 'CommandOrControl+Alt+C', () => toggleClickThrough());
